...
class Board extends React.Component {
    ...
    pickFromTreatGrid = grid => {
        const available = grid.filter(
            entry => !this.state.snake.some(x => x.every((value, index) => value === entry[index]))
        );
    ...
    tick = () => {
        const sameState = Object.keys(this.state)
            .map(key => this.state[key] === this.props.state[key])
            .every(x => !!x);
        if (!sameState) {
            const temp_state = {};
        ...
        return (
            <React.Fragment>
                <div
                    className="board"
                    style={{
                        height: 30 * this.props.state.size,
                        width: 30 * this.props.state.size,
                    }}
                >
                    {this.feedCells()}
                </div>
